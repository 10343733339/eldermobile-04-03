...
<div class="page-container">
	<div class="text-c"> 账号：
		<input type="text" class="input-text" style="width:250px" placeholder="账号" id="account" name="account">
...
$(document).ready(function() {
    ...
    "ajax": {
        ...
        "data": function (data) {//在此处对data（datatables传给服务器端的数据）进行处理 data.start是从哪个数据开始，data.length是页面长度 通过这两个参数可以分页 
            ...
            data.filter_LIKES_account = $("#account").val();
            data.filter_EQI_platform__id = '${userId }' != '1' ? '${platformId}' : '';
        },
    },
	"aoColumns" : [ //aoColumns设置列
	                {"data": null,"targets": 0,"sClass":"width50"},
	                {"data": "account", "orderable": false,'sClass': "text-c odd"},//data 数据 如果复杂逻辑可以写在回调fnRowCallback中否则可以直接返回aoData的属性
	                {"data": "nickname", "orderable": false,'sClass': "text-c odd"},
	                {"data": "telephone",  "orderable": false,'sClass': "text-c odd"},
	                {"data": "createTime", "orderable": false,'sClass': "text-c odd"},
	                {"data": function(data){if(data.rank==1) return "超级会员"; else if(data.rank==2) return "高级会员"; else return "普通会员";}, "orderable": false,'sClass': "text-c odd"},
	                {"data": function(data){if(data.status==1) return "启用"; else return "停用";}, "orderable": false,'sClass': "text-c odd"},
	                {"data": "roleName", "orderable": false,'sClass': "text-c odd"},
	                {"data": "loginCount", "orderable": false,'sClass': "text-c odd"},
	                {"data": "lastIp", "orderable": false,'sClass': "text-c odd"},
	                {"data": "lastTime", "orderable": false,'sClass': "text-c odd"},
	],
...
// 新增
function add(){
	var url = 'user/input.do';
	layer_show('添加用户',url,'1000','780');
}
...
function edit(){
	if(table.rows('.selected').data().length>0){
		...
		var url = 'user/input.do?id='+id;
		layer_show('编辑用户',url,'1000','780');
	}else{
