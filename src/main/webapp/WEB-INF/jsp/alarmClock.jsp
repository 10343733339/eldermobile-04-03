...
<div class="page-container">
	 <div class="text-c"> 
	事件名称：	<input type="text" class="input-text" style="width:250px" placeholder="事件名称" id="eventName" name="eventName">
...
$(document).ready(function() {
    ...
    "ajax": {
        "type": "POST",
        "url": 'alarmClock/search',
        "data": function (data) {//在此处对data（datatables传给服务器端的数据）进行处理 data.start是从哪个数据开始，data.length是页面长度 通过这两个参数可以分页 
            data.page = data.start / data.length + 1;
            data.rows = data.length;
            data.filter_LIKES_eventName = $("#eventName").val();
        },
    },
	"aoColumns" : [ //aoColumns设置列
	                {"data": null,"targets": 0,"sClass":"with50"},
	                {"data": "eventName", "orderable": false,'sClass': "text-c odd"},//data 数据 如果复杂逻辑可以写在回调fnRowCallback中否则可以直接返回aoData的属性
	                {"data": "eventContent", "orderable": false,'sClass': "text-c odd"},
	                {"data": function(data){if(data.status=='0') return "<span style='color:red'>已关闭</span>"; return "<span style='color:green'>已启用</span>";}, "orderable": false,'sClass': "text-c odd"},
	                {"data": "eventTime", "orderable": false,'sClass': "text-c odd"},
