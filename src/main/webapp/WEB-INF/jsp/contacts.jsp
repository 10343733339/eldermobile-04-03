...
<div class="page-container">
	 <div class="text-c"> 
	联系人姓名：	<input type="text" class="input-text" style="width:250px" placeholder="联系人姓名" id="contactName" name="contactName">
...
$(document).ready(function() {
    ...
    "ajax": {
        ...
        "data": function (data) {//在此处对data（datatables传给服务器端的数据）进行处理 data.start是从哪个数据开始，data.length是页面长度 通过这两个参数可以分页 
            ...
            data.filter_LIKES_contactName = $("#contactName").val();
        },
    },
	"aoColumns" : [ //aoColumns设置列
	                {"data": null,"targets": 0,"sClass":"with50"},
	                {"data": "contactName", "orderable": false,'sClass': "text-c odd"},//data 数据 如果复杂逻辑可以写在回调fnRowCallback中否则可以直接返回aoData的属性
	                {"data": "contactPhone", "orderable": false,'sClass': "text-c odd"},
	                {"data": "simMark", "orderable": false,'sClass': "text-c odd"},
	                {"data": function(data){if(data.isUrgent==0) return "否";else return "是";}, "orderable": false,'sClass': "text-c odd"},
	                {"data": "urgentLevel", "orderable": false,'sClass': "text-c odd"},
	                {"data": function(data){if(data.IsAutoAnswer==0) return "否";else return "是";}, "orderable": false,'sClass': "text-c odd"},
	                {"data": "createTime", "orderable": false,'sClass': "text-c odd"},
	                {"data": "updateBy", "orderable": false,'sClass': "text-c odd"},
	],
