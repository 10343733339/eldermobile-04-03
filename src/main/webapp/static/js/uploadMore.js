...
jQuery(function() {
	...
	$('.uploder-container').each(function(index){
        ...
        uploader[index] = WebUploader.create({
            ...
            server: '/multiple_tools/uploadFile?fileDir=appScreenshot',//上传地址
        ...
        uploader[index]	.on('ready',function(){
        			...
        			for(var i = 0; i < picc.length; i++){
                        ...
                        //把展示图片添加到input 回显
                       $("#picMany").append("<input type='hidden' value='"+$(picc[i]).val()+"' name='pic' id='success"+i+"Val'>");
        ...
        uploader[index].on('uploadSuccess',function(file,reponse){
        	...
        	if(reponse.code=='1')
        	  {
        			$("#picMany").append("<input type='hidden' value='"+reponse.obj+"' name='pic' id='"+file.id+"Val'>");
        	  }
        	else
        	{
        		alert(response.msg);
    ...
    function updateStatus(val,info,f_count,f_size) {
        ...
        if ( val === 'ready' ) {
            text = '选中' + f_count + '张图片，共' +
                    WebUploader.formatSize( f_size ) + '(只计算本次上传的)。';
        } 
